...
export default function EditMissionForm({ mission, onClose }: Props) {
  // Pre-fill address states from existing mission
  const toResult = (address: string, lat: number, lng: number): AddressResult => ({
    displayName: address, shortName: address, lat, lng,
  });

  const [pickupQuery,  setPickupQuery]  = useState(mission.pickupAddress);
  const [dropoffQuery, setDropoffQuery] = useState(mission.dropoffAddress);
  const [pickup,       setPickup]       = useState<AddressResult | null>(
    toResult(mission.pickupAddress, mission.pickupCoords.latitude, mission.pickupCoords.longitude),
  );
  const [dropoff,      setDropoff]      = useState<AddressResult | null>(
    toResult(mission.dropoffAddress, mission.dropoffCoords.latitude, mission.dropoffCoords.longitude),
  );
  ...
  function handlePickupSelect(r: AddressResult) {
    setPickup(r); setPickupQuery(r.shortName); pickupSearch.clearResults();
  }
  function handleDropoffSelect(r: AddressResult) {
    setDropoff(r); setDropoffQuery(r.shortName); dropoffSearch.clearResults();
  }

  async function onSubmit(values: FormValues) {
    if (!pickup || !dropoff) { setAddressError('Select both addresses.'); return; }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 3500, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', background: '#161616', borderRadius: '20px 20px 0 0', maxHeight: '92dvh', display: 'flex', flexDirection: 'column', animation: 'slideUp 0.25s cubic-bezier(0.32,0,0.67,0) both' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ overflowY: 'auto', padding: '20px 20px 40px', flex: 1 }}>
          ...
          <AddressField label="PICKUP ADDRESS" placeholder="Search pickup location…"
            ...
            results={pickupSearch.results} loading={pickupSearch.loading}
            selected={pickup} onSelect={handlePickupSelect}
            onClear={() => { setPickup(null); setPickupQuery(''); }} />

          <AddressField label="DROP-OFF ADDRESS" placeholder="Search drop-off location…"
            query={dropoffQuery} onQuery={setDropoffQuery}
            results={dropoffSearch.results} loading={dropoffSearch.loading}
            selected={dropoff} onSelect={handleDropoffSelect}
            onClear={() => { setDropoff(null); setDropoffQuery(''); }}
            error={addressError ?? undefined} />

          {/* Deadline */}
          <div style={{ marginBottom: 20 }}>
            <label style={{ color: '#888', fontSize: 11, fontWeight: 700, letterSpacing: 1, display: 'block', marginBottom: 8 }}>NEW PICKUP DEADLINE</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {DEADLINES.map((d) => (
                <button key={d.hours} type="button" onClick={() => setDeadlineHours(d.hours)}
                  style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: `1px solid ${deadlineHours === d.hours ? 'var(--color-primary)' : '#2a2a2a'}`, background: deadlineHours === d.hours ? 'var(--color-primary)' : '#0d0d0d', color: deadlineHours === d.hours ? '#fff' : '#888', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                  {d.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {[
              { field: 'itemPrice' as const, label: 'ITEM PRICE ($)' },
              { field: 'deliveryFee' as const, label: 'DELIVERY FEE ($)' },
            ].map(({ field, label }) => (
              <div key={field}>
                <label style={{ color: '#888', fontSize: 11, fontWeight: 700, letterSpacing: 1, display: 'block', marginBottom: 8 }}>{label}</label>
          ...
          <button type="submit" disabled={isSubmitting}
            style={{ width: '100%', padding: 14, borderRadius: 12, border: 'none', background: 'var(--color-primary)', color: '#fff', fontSize: 15, fontWeight: 700, cursor: isSubmitting ? 'not-allowed' : 'pointer', opacity: isSubmitting ? 0.7 : 1 }}>
